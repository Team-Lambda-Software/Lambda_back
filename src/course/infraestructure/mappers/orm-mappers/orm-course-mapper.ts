...
export class OrmCourseMapper implements IMapper<Course, OrmCourse>
{
    ...
    async fromPersistenceToDomain ( persistence: OrmCourse ): Promise<Course>
    {
        ...
        const course: Course =
            Course.create( persistence.id, await this.ormTrainerMapper.fromPersistenceToDomain(persistence.trainer), persistence.name, persistence.description, persistence.weeks_duration, persistence.minutes_per_section, persistence.level, sections, persistence.category_id, SectionImage.create( persistence.image.id, persistence.image.url ), tags, persistence.date)
