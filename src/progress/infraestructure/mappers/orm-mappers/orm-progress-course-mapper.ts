...
export class OrmProgressCourseMapper implements IMapper<ProgressCourse, OrmProgressCourse>
{
    ...
    {
        const persistenceProgress = OrmProgressCourse.create(domain.CourseId, domain.UserId, domain.IsCompleted, domain.CompletionPercent);
        return persistenceProgress;
    }

    async fromPersistenceToDomain(persistence: OrmProgressCourse): Promise<ProgressCourse> 
    {
        const domainProgress = ProgressCourse.create(persistence.user_id, persistence.course_id, persistence.completed, []);
