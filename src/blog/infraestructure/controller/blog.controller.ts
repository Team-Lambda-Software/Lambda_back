...
export class BlogController {
  ...
  constructor(
    @InjectModel('NotificationAddress')
    private addressModel: Model<OdmNotificationAddressEntity>,
    @InjectModel('NotificationAlert')
    private alertModel: Model<OdmNotificationAlertEntity>,
    ...
    private blogCommentModel: Model<OdmBlogCommentEntity>,
    @InjectModel('User') private userModel: Model<OdmUserEntity>,
  ) {
    this.imageTransformer = new ImageTransformer();
    this.notiAddressRepository = new OdmNotificationAddressRepository(
      addressModel,
    );
    ...
    this.odmBlogRepository = new OdmBlogRepository(blogModel, blogCommentModel);
    ...
    this.ormCategoryRepository = new OrmCategoryRepository(
      new OrmCategoryMapper(),
      dataSource,
    );
    this.ormTrainerRepository = new OrmTrainerRepository(
  ...
  async createBlog(
    @GetUser() user,
    @Body() createBlogParams: CreateBlogEntryDto,
  ) {
    ...
    for (const image of createBlogParams.images) {
      ...
      try {
        ...
        throw new BadRequestException(
          'Las imagenes deben ser en formato base64',
        );
      }
  ...
  @ApiBearerAuth()
  @ApiOkResponse({
    description: 'Devuelve la informacion de un blog dado el id',
    type: GetBlogSwaggerResponseDto,
  })
  async getBlog(@Param('id', ParseUUIDPipe) id: string, @GetUser() user) {
    const service = new ExceptionDecorator(
      new LoggingDecorator(
        new PerformanceDecorator(
          new GetBlogService(this.odmBlogRepository),
          new NativeLogger(this.logger),
        ),
        new NativeLogger(this.logger),
      ),
  ...
  @ApiBearerAuth()
  @ApiOkResponse({
    description: 'Devuelve la informacion de los blogs',
    type: SearchBlogsSwaggerResponseDto,
    isArray: true,
  })
  async searchBlogs(
    @GetUser() user,
    @Query() searchBlogParams: SearchBlogQueryParametersDto,
  ) {
    if (
      searchBlogParams.category ||
      (!searchBlogParams.category && !searchBlogParams.trainer)
    ) {
      const searchBlogServiceEntry: SearchBlogsByCategoryServiceEntryDto = {
        categoryId: searchBlogParams.category,
        userId: user.id,
        pagination: {
          page: searchBlogParams.page,
          perPage: searchBlogParams.perPage,
        },
      };
      ...
      if (searchBlogParams.filter == 'POPULAR') {
        ...
        const service = new ExceptionDecorator(
          new LoggingDecorator(
            new PerformanceDecorator(
              new SearchRecentBlogsByCategoryService(this.odmBlogRepository),
              new NativeLogger(this.logger),
            ),
            new NativeLogger(this.logger),
          ),
    ...
    const searchBlogServiceEntry: SearchBlogsByTrainerServiceEntryDto = {
      trainerId: searchBlogParams.trainer,
      userId: user.id,
      pagination: {
        page: searchBlogParams.page,
        perPage: searchBlogParams.perPage,
    ...
    if (searchBlogParams.filter == 'POPULAR') {
      const service = new ExceptionDecorator(
        new LoggingDecorator(
          new PerformanceDecorator(
            new SearchMostPopularBlogsByTrainerService(this.odmBlogRepository),
            new NativeLogger(this.logger),
          ),
          new NativeLogger(this.logger),
        ),
      ...
      const service = new ExceptionDecorator(
        new LoggingDecorator(
          new PerformanceDecorator(
            new SearchRecentBlogsByTrainerService(this.odmBlogRepository),
            new NativeLogger(this.logger),
          ),
          new NativeLogger(this.logger),
        ),
  ...
  @ApiBearerAuth()
  @ApiOkResponse({
    description: 'Devuelve la cantidad de blogs',
    type: GetCountResponseDto,
  })
  async getBlogcount(
    @GetUser() user,
    @Query() getBlogCountParams: GetBlogCountQueryParametersDto,
  ) {
    const service = new ExceptionDecorator(
      new LoggingDecorator(
        new PerformanceDecorator(
          new GetBlogCountService(this.odmBlogRepository),
          new NativeLogger(this.logger),
        ),
        new NativeLogger(this.logger),
      ),
    ...
    if (!getBlogCountParams.category && !getBlogCountParams.trainer) {
      throw new BadRequestException(
        'tiene que enviar o un entrenador o una categoria',
      );
    }
    const result = await service.execute({
      ...getBlogCountParams,
