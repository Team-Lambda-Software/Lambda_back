...
export class OdmTrainerRepository implements TrainerQueryRepository {
    ...
    constructor ( trainerModel: Model<OdmTrainerEntity>, userModel: Model<OdmUserEntity> )
    {
    ...
    async followTrainer ( trainerID: string, userID: string ): Promise<Result<string>>
    {
        try
        {
            ...
            if (odmTrainer == null)
            {
                return Result.fail<string>(new Error("Trainer not found"), 404, "Trainer not found");
            }
            const odmUser = await this.userModel.findOne( {id: userID} );
            if (odmUser == null)
            {
                return Result.fail<string>(new Error("User not found"), 404, "User not found");
            }
    ...
    async unfollowTrainer ( trainerID: string, userID: string ): Promise<Result<string>>
    {
        try
        {
            ...
            const userIndex = odmTrainer.followers.findIndex( follower => follower.id == userID );
            if (userIndex == -1)
            {
                return Result.fail<string>(new Error("User is not following this trainer"), 404, "User is not following this trainer");
            }
    ...
    async findTrainersByLocation ( latitude: string, longitude: string, pagination: PaginationDto ): Promise<Result<OdmTrainerEntity[]>>
    {
        try{
    ...
    async findTrainersByFollower ( followerID: string, pagination: PaginationDto ): Promise<Result<OdmTrainerEntity[]>>
    {
        try{
    ...
    async findAllTrainers ( pagination: PaginationDto ): Promise<Result<OdmTrainerEntity[]>>
    {
        try{
    ...
    async findAllTrainerFollowersId ( id: string, pagination: PaginationDto ): Promise<Result<string[]>>
    {
        try{
            ...
            const followersId: string[] = odmTrainer.followers.map( follower => follower.id );
    ...
    async getFollowerCount ( id: string ): Promise<Result<number>>
    {
        try{
            ...
            if (odmTrainer == null)
            {
                return Result.fail<number>( new Error("Trainer not found"), 404, "Trainer not found");
            }
    ...
    {
        try{
            const odmTrainers = await this.trainerModel.find( {"followers.id": userId} );
            if (!odmTrainers)
            {
    ...
    async checkIfFollowerExists ( trainerID: string, followerID: string ): Promise<Result<boolean>>
    {
        try{
            ...
            if (odmTrainer == null)
            {
                return Result.fail<boolean>( new Error("Trainer not found"), 404, "Trainer not found");
            }
            const followerExists = odmTrainer.followers.find( follower => follower.id == followerID );
