...
import { CreateTrainerEntryDto } from '../dto/entry/create-trainer-entry.dto';
...
import { CreateTrainerApplicationService } from 'src/trainer/application/services/commands/create-trainer.application.service';
...
import { TrainerCreated } from 'src/trainer/domain/events/trainer-created-event';
...
export class TrainerController {
  ...
  constructor(
    ...
    this.odmTrainerRepository = new OdmTrainerRepository(
      trainerModel,
      userModel,
    );

    this.trainerFollowQuerySyncronizer = new TrainerFollowQuerySyncronizer(
      this.odmTrainerRepository,
    );
    this.trainerUnFollowQuerySyncronizer = new TrainerUnFollowQuerySyncronizer(
      this.odmTrainerRepository,
    );
  ...
  @ApiBearerAuth()
  @ApiOkResponse({ description: 'Crea un nuevo entrenador.', type: String })
  async CreateTrainer(@Body() body: CreateTrainerEntryDto, @GetUser() user) {
    const service = new ExceptionDecorator(
      new LoggingDecorator(
        new AuditingDecorator(
          new PerformanceDecorator(
            new CreateTrainerApplicationService(
    ...
    this.eventBus.subscribe('TrainerCreated', async (event: TrainerCreated) => {
      ...
      const trainer = await this.trainerModel.create({
        ...
        first_last_name: event.name.firstLastName,
        second_last_name: event.name.secondLastName,
        email: event.email,
        phone: event.phone,
  ...
  @ApiOkResponse({
    description:
      'Devuelve informacion sobre un entrenador, todos sus seguidores, cursos y blogs que haya creado; dado su id.',
    type: GetTrainerProfileSwaggerResponseDto,
  })
  async GetTrainerProfile(
    ...
    const service = new ExceptionDecorator(
      new LoggingDecorator(
        new PerformanceDecorator(
          new GetTrainerProfileApplicationService(this.odmTrainerRepository),
          new NativeLogger(this.logger),
        ),
        new NativeLogger(this.logger),
      ),
    ...
    return {
      ...
      userFollow: value.doesUserFollow,
      location: value.trainerLocation,
    };
  }
  ...
  async ToggleFollowState(
    ...
    let baseService: IApplicationService<
      TogggleTrainerFollowServiceEntryDto,
      ToggleTrainerFollowServiceResponseDto
    >;

    const doesFollowResult = await this.trainerRepository.checkIfFollowerExists(
      id,
      user.id,
    );
    if (doesFollowResult.isSuccess()) {
      ...
      if (doesFollow) {
        baseService = new UnfollowTrainerApplicationService(
          this.trainerRepository,
          this.eventBus,
        );
      } else {
        baseService = new FollowTrainerApplicationService(
          this.trainerRepository,
          this.eventBus,
        );
      }
    ...
    const service = new ExceptionDecorator(
      new LoggingDecorator(
        new PerformanceDecorator(baseService, new NativeLogger(this.logger)),
        new NativeLogger(this.logger),
      ),
      new HttpExceptionHandler(),
    );
    ...
    if (baseService instanceof FollowTrainerApplicationService) {
      this.eventBus.subscribe(
        TrainerFollowed.name,
        async (event: TrainerFollowed) => {
          this.trainerFollowQuerySyncronizer.execute(event);
        },
      );
    } else
      this.eventBus.subscribe(
        TrainerUnfollowed.name,
        async (event: TrainerUnfollowed) => {
  ...
  @ApiOkResponse({
    description:
      'Obtiene todos los entrenadores existentes. Se puede filtrar a sólo aquellos que el usuario está siguiendo',
    type: GetManyTrainersSwaggerResponseDto,
    isArray: true,
  })
  //Gets all available trainers. May be filtered to only those that the user follows
  async GetManyTrainers(
    @Query() data: GetManyTrainersSwaggerEntryDto,
    @GetUser() user,
  ) {
    ...
    if (doFollowFilter == true) {
      baseService = new GetAllFollowedTrainersApplicationService(
        this.odmTrainerRepository,
      );
      //TEST
    } else {
      baseService = new GetAllTrainersApplicationService(
        this.odmTrainerRepository,
      );
      //TEST
    }
    ...
    const serviceDTO: GetManyTrainersServiceEntryDto = {
      userId: user.id,
  ...
  @ApiOkResponse({
    description:
      'Obtiene la cantidad de entrenadores que el usuario está siguiendo',
    type: GetUserFollowingCountSwaggerResponseDto,
  })
  ...
  async GetUserFollowingCount(@GetUser() user) {
    const service = new ExceptionDecorator(
      new LoggingDecorator(
        new PerformanceDecorator(
          new GetUserFollowingCountApplicationService(
            this.odmTrainerRepository,
          ),
          new NativeLogger(this.logger),
        ),
