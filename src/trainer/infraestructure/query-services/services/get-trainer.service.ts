...
export class GetTrainerService
  implements
    IApplicationService<
      GetTrainerServiceEntryDto,
      GetTrainerServiceResponseDto
    >
{
  ...
  async execute(
    data: GetTrainerServiceEntryDto,
  ): Promise<Result<GetTrainerServiceResponseDto>> {
    const resultTrainer = await this.trainerRepository.findTrainerById(
      data.trainerId,
    );
    if (!resultTrainer.isSuccess()) {
      return Result.fail<GetTrainerServiceResponseDto>(
        resultTrainer.Error,
